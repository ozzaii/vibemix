...
- **Use the `impeccable` skill for the visual polish pass** (Kaan directive 2026-05-21), not just default ui-phase/ui-review. Target Tier-1 live surfaces: the session/live view + the mascot overlay. Hold the locked **CDJ-Whisper** direction ([[project_visual_direction_cdj_whisper]]): 5 warm blacks, single amber accent (4 intensities), tactility via faint glow not faux-3D bevels, 20/80 rule, textured material feel, readability + restraint. **Typography = the IN-TREE direction: Saira + JetBrains Mono** (per `tauri/ui/.../DESIGN.md` + `tokens.css`). NOTE: the `[[project_visual_direction_cdj_whisper]]` memory note is STALE on type — it says "Geist + Fraunces", but Fraunces was explicitly removed in-tree. Hold Saira + JetBrains Mono; do NOT reintroduce Fraunces/Geist. Enforce via the project-local `frontend-enforcement` skill.
