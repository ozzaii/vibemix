...
- [x] **Phase 27: Eval Harness + v2.0 Carry-Forward Close-Out** — Autonomous hallucination proxy gate + sidecar universal2 + WASAPI subscription + Achird OPUS render + FLX4 sync sniff + `register_library` 5-min orphan patch. REQ count: 14. Pitfalls: P42, P43, P44, P45, P48, P63, P69, P70 (+ P46 legal-carveout prep). Shipped 2026-05-15 — 9/9 plans committed, 140 tests pass / 3 deferred (Windows-only COM + partial-state), `gsd-autonomous fully` mode.
- [x] **Phase 28: Library Intelligence v1** — Gemini Embedding 2 + sqlite-vec / numpy fallback · vibe search · "what's playing" grounding · drag-drop UI · 30-day staleness nudge. REQ count: 9. Pitfalls: P48, P54, P55, P56. Shipped 2026-05-15 — 9/9 plans committed, code-review fixes landed via 28-REVIEW-FIX, KAAN-ACTION-PROXY surface deferred.
- [x] **Phase 29: Post-Session Debrief MVP UI** — Chaptered review · 60–90s voiced TL;DR · 3 drills · clickable timeline · cited critique. REQ count: 9. Pitfalls: P81, P82. Shipped 2026-05-15 — 9/9 plans committed (29-00..29-08), cross-platform verdict + WAVE0 probes documented.
...
- [x] **Phase 37: Cross-Phase Integration Audit Gate** — `tests/e2e/test_seam_*` + integration audit script + orphan inventory + grey-area decision log. REQ count: 7. Pitfalls: P48, P66, P87. Shipped 2026-05-15 — 6/6 plans committed (37-01..37-06), 42 tests green; 5 cross-phase seams all WIRED (P18→P20, P19→agent, P25→P28, P27→eval-gate, P31→ws_bus); `.planning/v2.1-MILESTONE-AUDIT.md` artifact generated; AUDIT-VM + AUDIT-SIGN-VERIFY deferred to KAAN-ACTION-LEGAL.
- [x] **Phase 38: Signing Pipeline Real Execution** — Apple notarytool + SignPath GH Action wired with real secrets + post-sign verifier + Kaan local rehearsal script. REQ count: 7. Pitfalls: P46 (legal carveout — autonomous discharge FORBIDDEN). Shipped 2026-05-15 — 6/6 plans committed, 58 security tests green; DIST-09 (Francesco) + DIST-11 (Kaan) legal-capacity carveouts NEVER autonomously discharged (P46 audit enforces).
- [x] **Phase 39: Public RC Cut + Ship** — `cut_release.sh` 6-gate pre-flight · README hero `<video>` + feature-matrix auto-sync · 5-channel social publisher + NACK window · Discord launch flow · `sync_github_meta.sh` topics/description SoT · changelog auto-populator · 24h launch-rotation doc · Phase 16 override expiry gate. REQ count: 8. Pitfalls: P59, P68, P78, P79, P83, P85, P86, P87. Shipped 2026-05-16 — 8/8 plans committed (39-01..39-08), 91 tests green; cut_release.sh dry-run 5/6 gates pass (Gate 2 = unsigned binaries pending DIST-09 + DIST-11); 6 §SHIP customer-facing publishes + 3 §POST-RC-CLEANUP items deferred to KAAN-ACTION-LEGAL (NEVER autonomously discharged).
