...
- **UX-04 (actionable failure states):** P71's loud Python errors (`[FATAL]` exit-4 + `connection_error` surfacing) + `sidecar.rs` exit-code→reason mapping + `crash-banner.ts` + the in-session "AI SERVICE OFFLINE" foot + retry.
