...
**Out (historical v8.1 lock):** no new live-reaction AI provider, no new ws
port, no new IPC envelope; single-writer `MusicState` untouched (invariant #1).
Phase 90 later superseded the embedding side with local CLAP ONNX only, and
Viber set-prep/chat now uses local Codex for the current product path. Additive
— the existing `library curate` / Telegram CLI surfaces keep working; the four
cardinal invariants hold. **Ship, don't over-engineer:** the unification is
CONCRETE shared wiring (a module both surfaces import), not an abstract rewrite.
</domain>
...
- `library/` embedding store (current product path: local CLAP ONNX, 512-dim)
  + `next_suggestion.py` (the pill engine) + `telegram_bridge.py` (mobile
  transport).
...
- Additive gated design; single-writer `MusicState`; one socket; the shared-seam
  pattern (Phase 77/79 — one source both surfaces read); lazy imports to respect
  the memory storage-spine import boundary (Phase-77 `test_no_live_path_import`).
  Live reaction brain remains Gemini-routed; library embeddings are now local
  CLAP, and Viber set-prep/chat is local Codex in the current product path.
