...
### Embedding (on-device, via product CLAP embedder)
- Call the product `embed_factory.build_embedder()` / `ClapEmbedder` seam, which
  wraps local `ClapEngine(backend="onnx")` and returns 512-dim deterministic
  CLAP vectors. Mean-centering is mandatory downstream (CLAP is anisotropic).
...
- `.planning/research/clap-engine-deep-dive.md` — current pointer for CLAP engine
  decisions; use `docs/clap-engine.md` + `src/vibemix/library/clap_engine.py` as
  the implementation truth.
- `docs/clap-engine.md` — the product local CLAP ONNX engine contract.
...
- `src/vibemix/library/embed_factory.py` and `src/vibemix/library/embed_clap.py`
  — product embedder factory and CLAP cache strategy to mirror.
- `src/vibemix/library/embed.py` — legacy Gemini embedder kept only for
  migration/parity tests; do not use for new ingest paths.
- `src/vibemix/state/harmonics.py` — deterministic Camelot table.
</canonical_refs>

<scope_fence>
## DO NOT TOUCH (concurrent sessions own these)
- Coordinate before broad changes to `library/clap_engine.py` and
  `docs/clap-engine.md`; they are current product truth, not staged experiments.
...
- `EMBEDDING_DIM` is already 512 for the current CLAP product path. Store
  whatever `ClapEmbedder` returns and keep the fail-loud mismatched-store guard.
