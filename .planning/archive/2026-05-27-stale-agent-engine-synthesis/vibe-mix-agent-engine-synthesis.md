...
>
> **Current status:** historical pre-build synthesis. v8.2 Set Builder has since
> shipped; current product truth is in `.planning/v8.2-MILESTONE-AUDIT.md`,
> `.planning/phases/v8.2-STATUS.md`, and
...
The spec describes **Vibe Mix = Discovery + Sequencing**, a DJ set-prep engine. At
draft time vibemix had only the seed of this: a grounded library agent with
`search_vibe` / `get_track_features` / `create_playlist` plus a 1-step harmonic
next-track engine (`next_suggestion`). As of the v8.2 ship, Discovery
centroid/MMR/filters, Sequencing energy-curve/beam-search, the Energy model, and
Rekordbox export are implemented and wired through CLI, local Codex/MCP, and the
Library UI path.
...
|---|---|---|
| LLM reasoning = **Claude Sonnet** | No Anthropic API in product; current Viber/library chat + set-prep path uses local Codex over MCP/toolset for demo/test. Gemini remains live/TTS. | Reasoning layer = local Codex now; historical Gemini harness is not the product default. |
| Personalization = **Mem0** | **Mem0 REJECTED** → local profile/memory + sqlite-vec where needed. | Recency penalty = local `played_ids` / memory.db, not Mem0. |
| Audio embeddings = **StyleDNA / Pinecone** | **sqlite-vec local + CLAP ONNX 512D** for audio/text library embeddings. | Vector ops on the existing local store; no cloud vector DB. |
...
| Energy moat = **XGBoost regressor + 1001Tracklists scraping** | scraping/IP-moat = commercial; essentia is AGPL → poison. | v1 = narrow hand-rolled numpy/PyAV DSP formula. XGBoost/scraping = Bravoh, parked. |
...
**local library data + numpy/PyAV + local CLAP + local Codex + Apache-clean libs**. The spec's *data sources & business model*
...
- **Current product path:** local Codex/MCP over `LibraryToolset` drives Viber
  chat/set-prep/build-set. `library/agent.py` remains historical/legacy parity
  surface when present, not the default product brain for Library/Viber.
...
**Shipped in v8.2:** Discovery-pool-building (centroid/filters/MMR), Sequencing
(energy, curve, graph, beam search), and Rekordbox export. Pure-Python where
possible, local CLAP ONNX for embeddings, Apache-clean.
