...
- **Date:** 2026-06-01 (re-pinned 2026-06-02) · **Pinned HEAD:** `d8a2e556` (`feat(state): gate drop-call signal`; re-pin before judging if Codex lands again) · **Maintained by:** Claude read-only swarm.
- **★★★★★ READ FIRST (NEWEST) → [`CODEX_DIRTY-TREE-CLASSIFICATION.md`](../2026-06-02/CODEX_DIRTY-TREE-CLASSIFICATION.md)** — dirty/untracked tree classified @ HEAD `264af2c4`, then partially consumed by Codex. **LANDED:** the DROP *signal* layer is now committed as `d8a2e556` — `state/drop_predict.py` + `state/event_detector.py` + `tests/state/test_drop_predict.py` + `tests/state/test_event_detector_drop.py` (21 passed; `VIBEMIX_DROP_CALL` default OFF, one flag-gated producer, registry-observable without audio). **STILL HOLD:** the dirty `coach.py` DROP *speech* hunk — gate = (a) `filter_for_slop` is not called by the canned DROP line, (b) grounding-review on the un-cited speech text (Inv #2/#3), (c) live on-beat/false-positive artifact, (d) DROP cooldown. **Invariant to keep: exactly one DROP producer, flag-gated** (a second producer risks ungated speech). **DROP_IGNORE:** `.planning/eval-runs/*/` (large local proof archives, not release input) · strays `.cp_probe.txt` / `.mixxx-tmp-loop/` / `.planning/.tmp_filelist.txt`. **CLAUDE.md runtime guidance corrected at `d8a2e556+dirty-doc`:** product TTS is MOSS-only, passthrough ships silent (`PASSTHROUGH_GAIN=0.0`), and heartbeat is `180.0` + speak-gated. Lane raws: `_dirty-raw-*.md` + `_dirty-verify-drop-split.md`.
