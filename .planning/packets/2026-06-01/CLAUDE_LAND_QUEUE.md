...
- **Date:** 2026-06-01 (re-pinned 2026-06-02) · **Pinned HEAD:** `1ec73e9d` (`fix(runtime): fail closed on slop-filtered drop lines`; re-pin before judging if Codex lands again) · **Maintained by:** Claude read-only swarm.
- **★★★★★ READ FIRST (NEWEST) → [`CODEX_DIRTY-TREE-CLASSIFICATION.md`](../2026-06-02/CODEX_DIRTY-TREE-CLASSIFICATION.md)** — dirty/untracked tree classified @ HEAD `264af2c4`, then partially consumed by Codex. **LANDED:** the DROP *signal* layer is committed as `d8a2e556` — `state/drop_predict.py` + `state/event_detector.py` + tests (21 passed; `VIBEMIX_DROP_CALL` default OFF, one flag-gated producer, registry-observable without audio). **LANDED:** the fixed-line anti-slop sub-gate is committed as `1ec73e9d` — `runtime/drop_reaction.py::reaction_line()` now runs `filter_for_slop` and raises before speech if a future bank edit trips it (35 focused tests passed). **STILL HOLD:** the dirty `coach.py` DROP *speech* hunk — remaining gate = (a) grounding-review on the un-cited fixed speech text (Inv #2/#3), (b) live on-beat/false-positive artifact, (c) DROP cooldown. **Invariant to keep: exactly one DROP producer, flag-gated** (a second producer risks ungated speech). **DROP_IGNORE:** `.planning/eval-runs/*/` (large local proof archives, not release input) · strays `.cp_probe.txt` / `.mixxx-tmp-loop/` / `.planning/.tmp_filelist.txt`. **CLAUDE.md runtime guidance corrected at `9b3855c5`:** product TTS is MOSS-only, passthrough ships silent (`PASSTHROUGH_GAIN=0.0`), and heartbeat is `180.0` + speak-gated. Lane raws: `_dirty-raw-*.md` + `_dirty-verify-drop-split.md`.
