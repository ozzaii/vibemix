...
- **RESOLVED — voice provenance/licensing.** Kaan confirmed the co-host reference voice is cleared (not a cloned YouTube person; rights are good with the voice owner). This is NOT a ship-blocker and does not gate the Voice moves. The PyInstaller specs still do not bundle the ref/`.cache/vibemix`/chatterbox/mlx today, which is a separate packaging task, not a legal one.
