...
1. **MOSS NUKED — Chatterbox-Turbo cohost is the ONLY voice, ZERO-SHOT ref-clone (not finetuned)** (Kaan, "moss nuke sadece cohost ... zero shot olarak üretildiği, yapıldığı şekilde"). The pipeline already exists in `agent/chatterbox_tts.py` (base `chatterbox-turbo-8bit` via mlx-audio, temp 0.4, `resolve_ref_path` → `~/.cache/vibemix/cohost_voice_ref.wav`). Wire + bundle, NOT rebuild: (a) make the Kaan-locked pranker clip the production ref — source `~/Downloads/Prank_L4_t20 (1).wav`, music-stripped, placed at `~/.cache/vibemix/cohost_voice_ref.wav` and bundled into the app/DMG; (b) remove MOSS as engine/default/floor across the tree; (c) make the engine reachable on a packaged launch (`config_store.tts_engine` + env-seed at `__main__.py:1420`). **Platform consequence (Kaan-accepted, macOS-first):** mlx-audio is Apple-only, so Windows + Intel Macs have NO cohost voice once MOSS is gone — they ship voiceless / text-transcript-only as a tracked follow-up, NOT a MOSS floor. macOS Apple-Silicon ships with voice now. No MOSS, no cloud TTS, ever, as a fallback.
