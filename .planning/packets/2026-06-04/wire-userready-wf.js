...
(1) VOICE: ship DEFAULT engine = Chatterbox-Turbo (the ear-locked live voice). Engine reachable via config_store tts_engine + os.environ.setdefault at __main__.py:1420 (NOT env-only); MOSS = the never-mute floor when Chatterbox deps/model are absent. Packaging MUST bundle mlx-audio + the Chatterbox model/ref + clear the licensing check; if any of those cannot ship, MOSS is the automatic floor (never mute).
