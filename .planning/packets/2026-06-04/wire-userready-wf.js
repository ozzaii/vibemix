...
(1) VOICE: MOSS IS NUKED. The finetuned Chatterbox-Turbo cohost voice is the ONLY voice path - remove MOSS as engine, default, and floor across the codebase (tts_chain.py, local_tts.py MossLocalTTS, voice_presets, config_store, __main__.py, ui_bus, learn/runtime, _router_config, debrief, library/cost). The live agent renders ONLY the finetuned Chatterbox model (point at the finetuned model id, not the base chatterbox-turbo-8bit; bundle the finetuned weights + ref + mlx-audio + clear licensing). PLATFORM CONSEQUENCE (Kaan-accepted, macOS-first ship): mlx-audio is Apple-only, so on Windows + Intel Macs the cohost has NO voice once MOSS is gone - those targets ship voiceless / text-transcript-only as a TRACKED FOLLOW-UP (a portable finetuned voice lands later), NOT with a MOSS floor. macOS Apple-Silicon ships with the finetuned voice now. Do not reintroduce MOSS or any cloud TTS as a fallback.
