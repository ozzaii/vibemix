...
(1) VOICE: MOSS IS NUKED. The cohost voice = Chatterbox-Turbo produced EXACTLY THE WAY IT IS ALREADY BUILT - ZERO-SHOT ref-clone, NOT finetuned: base model chatterbox-turbo-8bit via mlx-audio, temp 0.4, conditioned on the Kaan-locked "pranker" reference clip. The pipeline already exists in agent/chatterbox_tts.py (configured_model / configured_temperature / resolve_ref_path -> _DEV_REF = ~/.cache/vibemix/cohost_voice_ref.wav). This is WIRE + bundle, NOT a rebuild: (a) make the pranker clip the production ref - source ~/Downloads/Prank_L4_t20 (1).wav, music-stripped, placed at ~/.cache/vibemix/cohost_voice_ref.wav AND bundled into the app/DMG so packaged users have it; (b) remove MOSS as engine/default/floor across the tree (tts_chain.py, local_tts.py MossLocalTTS, voice_presets, config_store, __main__.py, ui_bus, learn/runtime, _router_config, debrief, library/cost); (c) make the engine reachable on a normal/packaged launch (config_store tts_engine + os.environ.setdefault at __main__.py:1420, per the BACKEND map) since launchd strips env; bundle mlx-audio + the base model + the ref clip + clear licensing. PLATFORM CONSEQUENCE (Kaan-accepted, macOS-first): mlx-audio is Apple-only, so Windows + Intel Macs have NO cohost voice once MOSS is gone - they ship voiceless / text-transcript-only as a TRACKED FOLLOW-UP (a portable voice later), NOT a MOSS floor. macOS Apple-Silicon ships with the voice now. No MOSS, no cloud TTS, ever, as a fallback.
