...
- ✓ DJ-software 6-cell grid + canonical-10 controllers grid (reconciled to `midi/profiles/*.json` in v7.0 P68; was `midi/controllers/*.json` historically) + 4-gate a11y CI + 16 SVG placeholders — v3.0 (Phase 44 / LAUNCH-03 + LAUNCH-04)
...
| **Curated 10-controller MIDI library** + generic fallback | Covers the ~80% of mid-tier DJs (Pioneer DDJ family + Numark + Hercules) without forcing every user through a calibration wizard | ✓ Good — canonical 10 reconciled to `midi/profiles/*.json` (Phase 68 P68P01; was `midi/controllers/*.json` per LAUNCH-04 historically); v3.x Mixxx map transpile unlocks 30+ |
