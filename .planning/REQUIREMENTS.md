...
- [x] **VIS-04**: 5 `prep_*.glb` placeholder replacements via Mixamo retargets (ASSETS-MESHY-A/B + ASSETS-MIXAMO-RIG + ASSETS-PREP-REPLACE); total mascot bundle stays ≤ 25MB cap; per-clip 400KB-1.2MB. **Engineering scaffolding shipped (Plan 43-05):** `scripts/mascot/retarget_to_neon_rebel.py` + two-tier `scripts/mascot/check_bundle_size.sh` (25 MB total + 400 KB-1200 KB per-clip) + `scripts/mascot/MIXAMO-CLIP-SOURCES.md` + §VIS-04 Kaan-discharge runbook in `KAAN-ACTION-LEGAL.md`. Real GLB retargets are Kaan-discharge per §VIS-04 (Adobe-account-gated Mixamo download + Kaan-aesthetic-gated Pioneer-CDJ-headbob selection).
- [x] **VIS-05**: Mood→animation pool runtime validation — Hype-man / Teacher / Coach pools wired to MANIFEST clips; 30s smoke test per persona with crossfades; idle-zero contract bone-level tests pass
- [x] **VIS-06**: Mascot overlay perf — `data-blur-perf` honored on integrated GPU; `backdrop-filter` fallback ladder tested on Intel UHD + M1 base; 60fps p99 maintained
