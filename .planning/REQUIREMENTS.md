...
- [x] **MAST-02**: Once Competent, a skill's Mastered fill advances only when the live co-host detects the user performing that skill in a real session, mapped from existing EvidenceRegistry event types (MIX_MOVE / LAYER_ARRIVAL / harmonic / EQ-band MIDI / beatmatch) — no new detectors are invented.
- [x] **MAST-03**: Every live mastery credit must resolve a valid citation in `EvidenceRegistry`; an un-cited or fabricated event grants zero credit (Invariants #2 + #3, test-pinned).
