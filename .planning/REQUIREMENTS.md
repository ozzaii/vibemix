...
- [x] **GH-01**: The real `docs/assets/demo.mp4` (30-second hero film matching the CDJ-Whisper aesthetic) lands at the path the README `<video src>` already points at; `scripts/check_readme_hero_hash.py` updates from `sha256=PLACEHOLDER` to the real SHA-256 and stays GREEN; the asset is ≤ 8 MB; it plays inline on Chrome + Safari + Firefox; the `<img>` poster fallback also lands. §ASSETS-DEMO-CUT KAAN-ACTION discharged.
...
| GH-05 | Phase 70 | Shipped engineering-side 2026-05-24 (70P05 / Wave 4) — `tests/repo/test_github_presence.py` (26 checks: 16 offline + 10 `@pytest.mark.network` badge-200s, all 10 GREEN on the network grid). Pins: README badge URLs 200 (network-marked, deselected from default grid); demo-poster present + `demo.mp4` <=8388608B-or-PLACEHOLDER tolerance; `og-card.png` sha256 == MANIFEST (`test_og_card_present_and_hash_matches`, ROADMAP-SC3-named); README hero-hash via `check_readme_hero_hash.py::check()`; every `.github/ISSUE_TEMPLATE/*.yml` valid GitHub Forms (name/description/body; config.yml special-cased) — **reconciliation #1: validates the real `.yml` Forms shape, NOT `.md` `about:` front-matter**; PR template non-empty; 4 OSS files present + README-linked. **Phase 70 ENGINEERING-COMPLETE.** |
