...
- [x] **RECALL-02**: The coach prompt is grounded with **top-k (2–3 cap)** past moments via a **gated `recall[…]` block** in `state/coach.py::evidence_line` (copying the Phase 59 `decks[…]` gate verbatim so the cold/empty-memory golden stays **byte-identical**), behind a similarity floor (~0.7, mirroring `CITATION_THRESHOLD`) and event-gated to track-aware events.
