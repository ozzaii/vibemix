...
- [ ] **STORE-02**: A ~50-line `MemoryStore` wrapper exposes `add_record` + `query_topk`; each record carries `session_id` + timestamp + the raw text signature + its embedding — **never** an LLM-extracted "insight" (raw-in, raw-out). _(contract pinned RED-first by Plan 63-01; implementation lands in Plans 63-02/03)_
