...
- [x] **RENDER-04**: An AI highlight glow paints on a rendered control within 16 ms of receiving the `ipc.learn.highlight` envelope, composited via CSS-variable swap (no full SVG re-render); pinned by `tauri/ui/tests/learn/highlight-paint.test.ts`.
...
| RENDER-04 | Phase 92 — Lesson Runtime + Highlight Contract | Complete |
...
| LESSON-03 | Phase 92 — Lesson Runtime + Highlight Contract | Complete |
...
| EXEMPLAR-01 | Phase 93 — Exemplar Engine + `[exemplar:]` Evidence Source | Complete (P93-02 band_share_store + compute_band_shares + P93-04 ExemplarFinder.find() ranker + P93-06 ingest opt-in + CLI dispatch all SHIPPED) |
| EXEMPLAR-02 | Phase 93 — Exemplar Engine + `[exemplar:]` Evidence Source | Complete (P93-02 `_kick_correlation` + 0.8 threshold + spectral-leakage fix + P93-04 ranker filter `top_for_band(max_kick_corr=0.8)` + P93-06 ingest opt-in surface SHIPPED) |
