...
We already have research under `.planning/research/`: `oss-launch-2026-05-25/SUMMARY.md`, `clap-spike-2026-05-25/` (spike data), `viber-direction-2026-05-25/` (10 design docs incl. `app.md`, `cost.md`, `embedding.md`, `agent-harness-design.md`). Goal: make new-milestone CONSUME these instead of re-running the 4-researcher spawn, then hand to autonomous.
...
2. Ensure `.planning/research/SUMMARY.md` reflects the new vision. Either keep the canonical `research/SUMMARY.md` current, or synthesize a fresh `SUMMARY.md` from the three pre-written report dirs first (one `gsd-research-synthesizer` spawn pointed at `oss-launch-*/SUMMARY.md` + `viber-direction-*/*.md` + `clap-spike-*/`). The roadmapper will read whatever `SUMMARY.md` is at `.planning/research/SUMMARY.md`.
...
5. Commit, then run `gsd-autonomous` (or `--from <first> --to <last>`). With `workflow.skip_discuss=true` (config), autonomous turns each ROADMAP goal directly into a minimal CONTEXT.md and goes straight to planning — the leanest fully-autonomous grind. To force plan-phase to ground in the pre-written reports, reference them in each phase's ROADMAP goal/SC text and/or in the seeded CONTEXT.md so the planner's `@`-context includes `.planning/research/viber-direction-2026-05-25/<file>.md`.
