...
- [x] **Phase 61: Actionable-Not-Hype Coach Persona** — Extend the in-flight `live-tuning-or-brain` coach cells into prescriptive DJ notes (observed→impact→prescribe, real DJ verbs), wire the two new events, regression-fence the hype goldens, keep every note cited. ✅ Complete 2026-05-21 (4/4 COACH must-haves engineering-verified; hype goldens byte-stable; code-review 0 blocker. Live coach ear-pass + WR-01 EQ-naming live check = KAAN-ACTION).
- [x] **Phase 62: Floating Pill UI** — Clone `mascot_window.rs` → `pill_window.rs`: a transparent, on-top, draggable, non-focus-stealing pill on the existing ws:8765 frames, made primary via a tri-state `primary_surface` config; mascot kept secondary. Parallelizes with the spine. ✅ Complete 2026-05-22 (4/4 PILL must-haves engineering-verified; cargo 61/61 · vitest 787/787 · tsc clean · pytest 7-WIP baseline; code-review 2 blockers + 5/6 warnings fixed. Felt drag/focus/transparency/DMG/multi-monitor live-confirm = KAAN-ACTION).
...
**Plans**: 5 plans
- [x] 62-01-PLAN.md — Resolve-FIRST spike: pill_window.rs (transparent/on-top/draggable/non-focus-steal, geometry + display-change re-clamp, Accessory floor + NSPanel stretch) + "pill" capability allowlist [PILL-01, PILL-04]
