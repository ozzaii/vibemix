...
def _read_direct_midi_probe(path_raw: str, *, ran: bool, rc: int) -> dict:
    ...
    return {
        ...
        "frames": frame_count,
        "raw_messages": int((summary or {}).get("raw_messages") or frame_count),
        "unsupported_types": dict((summary or {}).get("unsupported_types") or {}),
        "diagnosis": (summary or {}).get("diagnosis"),
        "next_action": (summary or {}).get("next_action"),
