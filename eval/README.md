...
threshold-lock hashes, including the current dataset card, fixture version, and
parsed INTEL threshold values. It checks both scorecard provenance and the
provenance-validation stage. It also rejects artifacts where the fixture-audit
...
`scripts/eval/intel_gold.py validate` is the executable guard for those private
...
writing output, requires timestamps to be real UTC instants, requires `run_id`
dates to match entry timestamp dates, and can append it with `--append-log` only
...
CLI reports append write failures as structured non-zero JSON and withholds the
optional `--output` artifact. The optional `--output` artifact uses the same
atomic writer. Release promotion requires a passing scorecard plus a valid
hash-bound `intel_gate.py` artifact with matching threshold-lock and
threshold-values provenance.
...
timestamps, malformed canonical run ids, run-id/timestamp date mismatches, stale
