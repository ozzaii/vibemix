...
def test_release_yml_repairs_macos_sidecar_symlinks_before_signing(workflow_yaml):
    ...
    assert "--require-chatterbox-source" in steps[verify_index]["run"]
    ...
    assert "--require-chatterbox-source" in steps[dmg_verify_index]["run"]
    ...
    assert "--require-chatterbox-source" in steps[updater_verify_index]["run"]
