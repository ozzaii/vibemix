...
factual UI affordance labelling (not co-host speech), and it states the real next
step at every stage.
"""
...
def test_competent_beatmatching_counts_down_after_producer_lands():
    # The owned-deck practice loop now emits cited BEATMATCH_GRADED, so the wall
    # can offer the same grounded Mastered path as the other creditable skills.
    progress = LearnProgress()
