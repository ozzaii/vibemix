...
"""DEBRIEF-04: synthesized Chatterbox TLDR audio is MP3 (libmp3lame via PyAV).

Real Chatterbox model calls are out of scope for offline tests — we verify the PyAV
...
def test_synthesize_chatterbox_mp3_happy_path():
    """End-to-end: mocked Chatterbox line audio → PCM → MP3 bytes."""
    adapter = object()
...
def test_synthesize_chatterbox_mp3_raises_on_empty_audio():
    """Chatterbox returns no audio → typed error."""
...
def test_synthesize_chatterbox_mp3_raises_on_line_synth_exception():
...
def test_synthesize_chatterbox_mp3_raises_on_unavailable_model():
    def missing_model_factory():
        raise ChatterboxUnavailable("missing Chatterbox runtime")

    with pytest.raises(DebriefGenerationError) as ei:
        synthesize_chatterbox_mp3("x", adapter_factory=missing_model_factory)
    assert ei.value.reason == "tldr_generation_failed"
    assert "Chatterbox TTS unavailable" in ei.value.message
