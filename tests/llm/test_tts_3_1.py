...
The live product voice is local MOSS-only. This file used to pin Gemini 3.1
Flash TTS router paths and VCR cassette scaffolding; the MOSS-era contract is
the opposite:

- cloud TTS router aliases are invalid;
- paid TTS rows may still exist only as explicit cost-model what-ifs;
- the legacy prompt tag DSL remains an opt-in prompt block, and live MOSS
...
def test_cloud_tts_router_aliases_are_retired(path: str) -> None:
    """MOSS is the only product voice; cloud TTS is not a model-router path."""
...
def test_tag_dsl_is_suppressed_for_live_moss_prompt_shape() -> None:
    """Live MOSS callers opt out of the DSL, so tags do not reach speech."""
