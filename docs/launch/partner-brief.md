...
- **Set Builder** — Viber agent discovers a pool, sequences it on an energy curve, explains every transition, exports straight to Rekordbox
- **Beginner Module** (in build, v9.0) — interactive teaching mode that renders the user's actual controller on screen and walks them through 36 grounded lessons, with the library playing exemplar tracks so they *hear* what each EQ band does

## The unfair angle

Every reaction is cited against real evidence — the audio chunk, the MIDI move, the current track, the screen state. Un-cited commentary strips to a one-word ack. That anti-slop discipline is the moat: a generic LLM wrapper can't get there from prompting alone, and DJs spot fake instantly. The grading bar is *"real friend in your ear"*, not *"voice assistant doing music commentary"*.
...
- **Controller manufacturers** — official mapping verification, co-branded launch
- **DJ software vendors** — deeper integration past the audio + screen grounding floor
- **DJ schools / educators** — the Beginner Module as a teaching surface
- **Distribution / community** — Discord, festival activations, influencer DJs on the bring-up rota

---

If you want a private demo on real hardware, ping **kaan@bravoh.ai**.
