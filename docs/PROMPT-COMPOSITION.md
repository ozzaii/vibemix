...
| `TRACK_CHANGE`            | `src/vibemix/state/event_detector.py:273` | 5.0                                       | no                  | `ev`, `aud`, `mix`, `track`, `recall`, `energy` (see Sec. 4) |
| `PHASE`                   | `src/vibemix/state/event_detector.py:289` | 10.0                                      | no                  | `ev`, `aud`, `mix`, `recall`, `energy` (see Sec. 4)       |
...
| `TRANSITION_OPPORTUNITY`  | `src/vibemix/state/event_detector.py:443` | 20.0                                      | no                  | `ev`, `key`, `mix`, `cue`, `energy` (see Sec. 4)          |
| `HEARTBEAT`               | `src/vibemix/state/event_detector.py:469` | 45.0 (`HEARTBEAT_SEC`)                    | yes                 | `ev`, `aud`, `mix`, `energy` (see Sec. 4)                 |
...
- "Citation sources (may carry)" lists which of the 12 `EVIDENCE_SOURCES` tokens (locked frozenset at `src/vibemix/state/evidence_registry.py:129`) the event MAY cite. The exact body grammar per source and the per-event-type association rationale live in Sec. 4 (Plan 101-02). The 12 sources are: `ev`, `aud`, `midi`, `track`, `screen`, `mix`, `key`, `recall`, `exemplar`, `cue`, `judge`, `energy`. `screen` is populated by the deck-poller screen-capture subsystem and may flow into the prompt on any event when present; it is not gated per event type. `energy` is written by the energy-read receipt (`src/vibemix/runtime/energy_read_voice.py`) at receipt-build time on `HEARTBEAT`/`PHASE`/`TRACK_CHANGE`/`TRANSITION_OPPORTUNITY` — the speak-gate requires that receipt before the describe-bank events reach the brain, and on its scorer-transition path the receipt also registers `track`/`mix` keys.
...
The ack events are short, frequent, and structurally compressible -- a HEARTBEAT does not benefit from a registry corpus footer the way a TRACK_CHANGE does. The compressed prompt lands a fast vocal acknowledgment through the same streaming generation path as every other event, trading the corpus footer and recall fragment for >=500ms TTFT instead of the full reasoning-grounded reaction the heavy events get. There is NO pre-canned acknowledgment substitution here: the ack-bank was retired 2026-05-19 (strip-to-silence replaced the pre-recorded "yeah/oh/nice" clips -- they injected English placeholders into Turkish sessions and fought the anti-slop thesis; see the `src/vibemix/runtime/coach.py` module header and the strip block at `src/vibemix/agent/dj_cohost.py`). The full-prompt path is preserved for every other event type so substantive reactions stay grounded.
