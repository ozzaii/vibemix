...
production voice path is **local Chatterbox-only**: there is no Cartesia, Gemini TTS,
...
default stack. The voice provider bill is **€0.00** because Chatterbox runs on-device.
The dominant paid leg is now listening/audio input into the Gemini brain.

Chosen stack: brain `live_coach_cand_25flash`, voice **Local Chatterbox**, listening
...
| tts | **0.00** | **0** | local Chatterbox voice; no provider bill |
| Viber set-prep | 0.02 | 227 | off-session set building and library questions |
| LiveKit | 0.00 | 0 | local direct mode; no Cloud room |

The paid center of gravity moved from speech to listening. Chatterbox removes
the old cloud-TTS bill; it does not remove the need to package the approved
reference clip and prove the voice on a real release artifact.
...
|---|---:|---|
| **Local Chatterbox (production)** | **9,917** | current default; provider TTS bill is zero |
...
1. **Listening window and brain tier.** With Chatterbox local, audio-input tokens are
...
| Local Chatterbox | voice | `vibemix.agent.chatterbox_tts` | verified as zero provider bill |
...
- This proves source-level cost math, not release readiness. A shippable app must
  still prove Chatterbox reference availability, packaged sidecar behavior, and
  live audio output on a clean machine.
- The 90% cache-hit assumption is not yet a live-measured product fact.
- This proves cost, not taste. Whether the chosen brain and Chatterbox voice feel like
