...
2026-06-01. La voce di produzione è **solo Chatterbox locale**: nella catena live non
...
costo del provider voce è **€0.00** perché Chatterbox gira on-device. La voce di costo
dominante è ora l'ascolto/audio-input verso il cervello Gemini.

Stack scelto: cervello `live_coach_cand_25flash`, voce **Local Chatterbox**, ascolto
...
| tts | **0.00** | **0** | voce Chatterbox locale; nessun costo provider |
...
Il centro di gravità del costo si è spostato dalla voce all'ascolto. Chatterbox
toglie il vecchio costo cloud-TTS; non toglie il bisogno di distribuire la clip
di riferimento approvata e provarla su un artifact release reale.
...
|---|---:|---|
| **Local Chatterbox (produzione)** | **9,917** | default corrente; costo provider TTS zero |
...
1. **Finestra di ascolto e tier del cervello.** Con Chatterbox locale, i token
...
| Local Chatterbox | voce | `vibemix.agent.chatterbox_tts` | verificato come costo provider zero |
...
- Questo prova la matematica del costo a livello sorgente, non la readiness di
  release. Un'app distribuibile deve ancora provare disponibilità della clip
  Chatterbox, comportamento sidecar packaged e audio live su macchina pulita.
- Il 90% di cache-hit non è ancora un fatto prodotto misurato live.
- Questo prova il costo, non il gusto. Se il cervello scelto e la voce Chatterbox
...
vibemix library budget --stack live --dau 10000 --brain live_coach_cand_25flash --tts chatterbox-local
...
vibemix library budget --stack live --dau 10000 --brain live_coach_cand_25flash --tts chatterbox-local --cache-hit 0.0
vibemix library budget --stack live --dau 10000 --brain live_coach_cand_25flash --tts chatterbox-local --sessions-per-month 4
```
