...
**Phase 43 / VIS-09** — engineering-side deliverables for Francesco's capture
day. The deterministic demo-mode sequencer has been retired: current launch
claims must be backed by a real source-run or packaged-build proof run, not
scripted playback. These docs now preserve the booth, camera, audio, and
editing plan for a live vibemix capture.
...
| [`AUDIO-CAPTURE.md`](./AUDIO-CAPTURE.md) | 3-track capture plan (AI voice + ambient + headphone-return) + clapboard sync + AV spec. |
...
These are the load-bearing visual contracts Kaan + Francesco must hold the line
on. Engineering supplies the live source-run or packaged-build proof; aesthetic
judgment closes the gate.
...
- [x] Deterministic demo-mode sequencer retired; current claims require live proof
