# Vendored verbatim from OpenMOSS / MOSS-TTS-Nano (https://github.com/OpenMOSS/MOSS-TTS-Nano),
# Apache License 2.0 (c) OpenMOSS Team, Fudan University, SII and MOSI, 2026.
# This is the repo's torch-free streaming CPU engine: it imports ONLY the stdlib
# + numpy + onnxruntime (no torch/transformers), which is why it fits vibemix's
# torch-free local-inference posture (same as the CLAP/CUE ONNX engines). It is
# the engine the local-TTS bench measured at 71ms TTFT / 0.18 RTF on M4 Max CPU.
# Kept byte-for-byte to ease upstream re-sync; do NOT refactor — wrap it in
# vibemix.agent.local_tts instead. See docs / NOTICE for attribution.
