...
"""One-shot synth seam — drive the existing Chatterbox live TTS chain for one line.

The live co-host voice is a LiveKit ``tts.FallbackAdapter`` with one local
provider. The viral auto-mix demo has no room, but the reel is
...
``build_default_line_adapter`` is the live wiring. Same chain, same product voice.
"""
...
def build_default_line_adapter():
    """Build the live Chatterbox-only TTS FallbackAdapter from local model state."""
