...
def _build_library_subparsers(parser: argparse.ArgumentParser) -> None:
    ...
    sp_ingest = sub.add_parser(
        ...
        description=(
            "Detect a Rekordbox collection.xml, Traktor collection.nml, "
            "VirtualDJ database.xml, or Engine DJ m.db at its standard export "
            "location (or pass an explicit path), parse each track, embed it ON-DEVICE via CLAP "
    ...
    sp_ingest.add_argument(
        ...
        help=(
            ...
            "VirtualDJ database.xml, or Engine DJ m.db (omit to auto-detect the "
            "selected source's standard export location)"
        ),
    )
    sp_ingest.add_argument(
        "--source",
        choices=("rekordbox", "traktor", "virtualdj", "engine", "serato"),
        default="rekordbox",
...
def _cmd_library_ingest(args: argparse.Namespace) -> int:
    ...
    if source_name == "traktor":
        ...
        source = EngineDJSource(database_path=explicit) if explicit else EngineDJSource()
        missing_msg = (
            "[FATAL] library ingest: no Engine DJ m.db found. Point vibemix at "
            "your Engine DJ Database2/m.db, then re-run "
            "`vibemix library ingest --source engine <path>` (or place it at a "
            "standard location)."
        )
    elif source_name == "serato":
        source = SeratoSource(library_path=explicit) if explicit else SeratoSource()
        missing_msg = (
            "[FATAL] library ingest: no Serato _Serato_/database V2 found. Point "
            "vibemix at your Serato database V2 file, then re-run "
            "`vibemix library ingest --source serato <path>` (or place it at a "
            "standard location)."
        )
    else:
