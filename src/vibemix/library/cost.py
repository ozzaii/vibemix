...
high implicit cache-hit rate the input leg collapses, but output + TTS-audio
never cache, so TTS dominates the per-turn bill. All money is USD here; the EUR
conversion happens at the CLI boundary.
"""
...
class LiveStackSpec:
    ...
    premium tiers) so `budget --stack live` shows today's bill; flags swap in the
    cheaper candidates to reveal the lever."""

    live_brain_path: str = "live_coach"
    tts_path: str = "live_coach_tts"  # a router path OR a vendor model id
