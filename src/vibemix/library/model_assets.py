...
def install_models(
    ...
    ``"required"`` is the app's first-run setup path and currently installs the
    required CLAP snapshot only. CUE-DETR downloads only when a release/ops
    build provides an HTTPS artifact URL plus size/SHA pins. Otherwise that
    target reports an actionable manual setup error.
    """
