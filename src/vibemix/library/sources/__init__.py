...
Traktor ``collection.nml``, VirtualDJ ``database.xml``, Engine DJ ``m.db``;
Serato later) onto the shared
...
``base`` module. Concrete sources (``rekordbox``, ``traktor``, ``virtualdj``,
``engine``) are imported on demand by the ingest path / CLI, never eagerly here
— keeps the import surface lean and the heavy-dep-free contract intact for CI.
"""
