...
Serato ``database V2``) onto the shared
...
``engine``, ``serato``) are imported on demand by the ingest path / CLI, never
eagerly here — keeps the import surface lean and the heavy-dep-free contract
intact for CI.
"""
