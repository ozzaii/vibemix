...
_RULES_BLOCK = (
    ...
    "or guess them. If a tool returns bpm/camelot as null, or returns "
    "metadata_warnings, say the library metadata is missing and do not treat "
    "that BPM/key range as verified.\n"
    "3. To inspect several candidates, call inspect_candidates ONCE with all "
    "track_ids instead of looping get_track_features / get_track_sections / "
    "get_track_energy per track.\n"
    "4. When you have chosen the tracks, return them as the final JSON object "
    ...
    "5. Keep it tight — a focused set beats a padded one."
)
...
_BUILD_SET_RULES = (
    ...
    "sequence_set → export_set. Do NOT inspect every candidate one by one: "
    "sequence_set already resolves BPM, key, stored vectors, and perceived energy "
    "for the whole pool deterministically.\n"
    "If you do need candidate receipts, call inspect_candidates ONCE with all "
    "track_ids; never loop get_track_features / get_track_sections / "
    "get_track_energy across the pool.\n"
    "WORKFLOW (in order):\n"
    ...
    "guess them. If a tool returns bpm/camelot as null, or returns "
    "metadata_warnings, say the library metadata is missing and do not treat "
    "that BPM/key range as verified. Add one actionable line: re-import or run "
    "library ingest from the DJ library/source so Viber can use DJ-analyzed "
    "BPM/key next time.\n"
...
_CHAT_RULES_BLOCK = (
    ...
    "and do not treat that BPM/key range as verified. Add one actionable line: "
    "re-import or run library ingest from the DJ library/source so Viber can "
    "use DJ-analyzed BPM/key next time.\n"
    "3. To inspect several candidates, call inspect_candidates ONCE with all "
    "track_ids instead of looping get_track_features / get_track_sections / "
    "get_track_energy per track.\n"
    ...
    "6. Ground a web or technique claim with the matching tool "
    "(web_search / retrieve_dj_knowledge) — let the source "
    "show.\n"
    "7. You do NOT have to call a tool every turn; if they're just chatting, "
    "chat back.\n"
    "8. When done, return the final JSON {reply, tools_used, tool_trace, "
