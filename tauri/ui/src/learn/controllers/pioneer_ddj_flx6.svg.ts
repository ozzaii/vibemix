...
//
// Phase 91 Plan 06 — DDJ-FLX6 inline-SVG controller schematic.
//
// Provenance: Hand-authored from the DDJ-FLX6 "Operating Instructions"
...
// vendor logo, NO faceplate photo-lift, NO brand-orange). The FLX6 is the
// physical-2-channel sibling of the FLX10 with a deck-select toggle that
// addresses 4 software decks; per the vibemix profile (`decks: ["A","B"]`)
// we render the deck A/B routing only — the toggle itself is not a
// mappable control surface.
//
...
//
// Layout follows the FLX-family convention (FLX4 + FLX6 share the same
// physical chassis topology — 2 decks with Pioneer-spec jog wheels +
// 3-band EQ stacks + filter knobs + crossfader + smart-fader bipolar
// per deck mapped here as `filter` for parity with the Color FX seam):
//
...
// binding in `src/vibemix/midi/profiles/pioneer_ddj_flx6.json`, and
...
export const PIONEER_DDJ_FLX6_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1280 720" role="img" aria-label="Pioneer DDJ-FLX6 schematic" class="learn-controller-schematic">
  ...
  <!-- Filter A — single bipolar knob below EQ stack (FLX6 Smart Fader knob) -->
