 ...
 * sentinel — especially "api-key-missing" (exit-4), the #1 "co-host never
 * speaks" cause: P71 made the Python side fail LOUD with exit 4, sidecar.rs
 * maps exit 4 → reason "api-key-missing", and this is the UI end of that loop.
...
describe("showFatalBanner (DOM)", () => {
  ...
  it("reveals the banner with the api-key-missing guidance", () => {
    ...
    expect(line.textContent).toContain("GEMINI_API_KEY");
  });
});
