...
//                    FAILS RED at 80 ms (triggers §LEARN-LATENCY-CONTINGENCY).
//
// Phase 91 Plan 02 — verbatim ~50-line skeleton from RESEARCH §Code Example 3
// with one defensive seam: the `PIONEER_DDJ_FLX4_SVG` import is wrapped in a
// dynamic `import()` so this spec collects + skips cleanly today (Plan 05
// lands the SVG string). The harness body, constants, and 240-sample triangle
// sweep are exactly as the research code example specifies.
//
// When Plan 05 lands `tauri/ui/src/learn/controllers/pioneer_ddj_flx4.svg.ts`,
// flip the `it.skip(...)` to `it(...)` — no other test-body changes needed.
...
describe("midi_position → DOM mirror latency", () => {
  beforeAll(() => {
    // Reset the test-env document; Plan 05's executor will populate
    // `#learn-stage` with the production renderer when the SVG file
    // lands. For Wave-0 RED-state, the stage is empty (test skips).
    document.body.innerHTML = `<div id="learn-stage"></div>`;
    if (!SVG_PRESENT) {
      return;
    }
    // Read the SVG string directly via fs so Vite's import-analysis can't
    // try to statically resolve a module path that doesn't exist yet
    // (Plan 05 lands the .svg.ts file). When the file lands, this read
    // returns the source text of the module; the regex below extracts the
    // backtick-delimited SVG body. Plan 05's executor MAY swap this to the
    // canonical static import once the file exists — the test contract is
    // the harness shape, not the import path.
  ...
  testFn("P95 latency ≤ 50 ms target; FAILS RED at 80 ms", () => {
